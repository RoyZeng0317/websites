...
def _fetch_fundamentals(symbol: str, current_price: float = 0) -> dict:
    ...
    if result and (result.get("dividendYield") is None or result.get("dividendRate") is None):
        try:
            ...
            _div_url = f"https://query1.finance.yahoo.com/v8/finance/chart/{urllib.parse.quote(symbol)}?range=5y&interval=1mo&events=div"
            _div_rsp = requests.get(_div_url, headers={"User-Agent": _ua}, timeout=10)
