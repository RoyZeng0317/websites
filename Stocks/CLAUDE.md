...
- `GET/PUT/DELETE /api/admin/stock/{symbol}` — admin-only manual override for fields the live fetch gets wrong (marketCap, dividendNote, dividendYield, peRatio, priceToBook, name, sector, industry, website, description). `GET /api/stock/{symbol}` merges these in before returning (see `_load_stock_override` in `main.py`). Auth + persistence pattern is identical to `/api/admin/news/*` (see `backend/admin_stock_info.py` and `backend/admin_news.py`): Firebase ID Token (`Authorization: Bearer`) verified via `google-auth` against `ADMIN_EMAIL`, writes go to a local JSON file (`backend/data/stock_overrides/{symbol}.json`) + best-effort commit back to GitHub via `GITHUB_TOKEN` (Render's disk is ephemeral, wiped on every redeploy). Edited locally with `frontend/src/components/StockInfoEditor.py` (tkinter desktop tool, not part of the deployed frontend) — grab a token via the "複製管理員 Token" button in `NewsAdminPanel.tsx`.
...
[x] frontend\src\components\StockInfoEditor.py 讓開發者手動編輯股票資訊（市值/配息配股/殖利率/本益比/股價淨值比/公司基本資料） — 2026-07-31: 原檔只是空殼（`path = "backend\\data\\"` 底下根本沒有股票資訊檔案）。討論後決定：資料不落地在本地檔案，改走後端 API，且直接沿用 `admin_news.py` 已經在跑的模式（Firebase ID Token 驗證 + 本地 JSON 寫入 + commit 回 GitHub 撐過 Render 重新部署），不需要另外申請 Firestore service account 金鑰。新增 `backend/admin_stock_info.py`（`/api/admin/stock/{symbol}` GET/PUT/DELETE，見上方 Key endpoints）、`main.py` 的 `get_stock_info()` 回傳前合併 override、`NewsAdminPanel.tsx` 加「複製管理員 Token」按鈕、`StockInfoEditor.py` 重寫成可用的 tkinter 工具。**待處理**：`GITHUB_TOKEN`/`ADMIN_EMAIL` 需已設定在 Render 環境變數（`admin_news.py` 應該已經在用，未確認的話先用 `GET /api/admin/news/dates` 測試 token 是否有效）；尚未實機驗證 tkinter 工具能否成功打 API（僅語法檢查通過）。2026-07-31 更新：根因找到——`admin_stock_info.py`／`StockInfoEditor.py` 當時是未加入版控的新檔（`git status` 顯示 `??`），從未 push 到 GitHub，Render 從沒部署過，導致 `/api/admin/stock/{symbol}` 404 Not Found（實際上 token 驗證是通過的）。已 commit + push（commit `77fec04`），Render 會自動重新部署。**仍待你實測**：部署完成後重跑 StockInfoEditor.py 讀取/儲存，確認 GITHUB_TOKEN/ADMIN_EMAIL 是否已設好（若儲存後 `github.committed` 是 false，再依 `reason` 排查）。

## Error log
[x] frontend show: ```無法載入新聞資料 Unexpected token '<', "<!DOCTYPE "... is not valid JSON ``` this error log — 2026-07-24: root cause was the raw HTML error page (from Render free-tier cold start / gateway hiccup) being passed straight to `res.json()`. `News.tsx fetchNews()` now checks `content-type` before parsing; non-JSON or network failures auto-retry up to 3× with backoff (3s/6s) to ride out cold starts, real JSON error responses (e.g. 404 no-data) show immediately without retry, and final fallback shows a friendly Chinese message instead of the raw parse error. Also fixed: `.notif-window` had `margin-top: 50%` (percentage vertical margins resolve against containing-block **width**, not height) pushing the notification panel off-screen with no way to scroll to it — replaced with overlay `padding-top` + `overflow-y: auto` + `max-height: calc(100dvh - 72px)`. Both deployed to stocks-global.web.app.
