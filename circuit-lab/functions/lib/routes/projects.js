...
router.get('/', isAuthenticated, async (req, res) => {
    try {
        const projects = await getRows(`SELECT p.*, u.name as owner_name
       FROM projects p
...
router.get('/:id', async (req, res) => {
    try {
        const project = await getRow('SELECT p.*, u.name as owner_name FROM projects p LEFT JOIN users u ON p.owner_id = u.id WHERE p.id = $1', [req.params.id]);
        if (!project) {
...
router.post('/', isAuthenticated, async (req, res) => {
    try {
        ...
        await run('INSERT INTO projects (id, name, description, owner_id, data, is_public) VALUES ($1, $2, $3, $4, $5, $6)', [id, name ?? 'Untitled Project', description ?? '', req.user.id, JSON.stringify(data ?? {}), isPublic ?? false]);
...
router.put('/:id', isAuthenticated, async (req, res) => {
    try {
        ...
        const collab = await getRow('SELECT 1 FROM project_collaborators WHERE project_id = $1 AND user_id = $2', [req.params.id, userId]);
        if (!isOwner && !collab) {
            return res.status(403).json({ error: 'Not authorized' });
        }
        const { name, description, data, isPublic } = req.body;
        await run(`UPDATE projects SET
