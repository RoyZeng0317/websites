...
function SerialMonitorPanel() {
  ...
  const lines: string[] = [];
  if (!simulationState) {
    lines.push('// Serial Monitor — press ▶ Simulate to see output');
  } else {
...
function WaveformsPanel() {
  ...
  return (
    <div style={panelStyles.waveforms}>
      <div style={panelStyles.sectionTitle}>Node Voltages</div>
      {nodes.length === 0 && (
...
function PropertiesPanel() {
  ...
  return (
    <div style={panelStyles.props}>
      ...
      {def?.description && <div style={panelStyles.propsDesc}>{def.description}</div>}

      <div style={panelStyles.sectionTitle}>Properties</div>
          ...
          <div key={key} style={panelStyles.propRow}>
            ...
            {isBool ? (
              ...
              <input
                ...
                value={String(val)}
                onChange={(e) =>
                  updateProperty(comp.id, key, isNum ? parseFloat(e.target.value) || 0 : e.target.value)
                }
              />
            )}
          </div>
      ...
      {def?.ports && def.ports.length > 0 && (
        <>
          <div style={panelStyles.sectionTitle}>Ports</div>
          {def.ports.map((port) => (
      ...
      <div style={panelStyles.sectionTitle}>Position</div>
      <div style={panelStyles.propRow}>
        <label style={panelStyles.propLabel}>x / y / z</label>
        <span style={{ fontSize: 12, color: '#aaaacc', fontFamily: 'monospace' }}>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  canvas:     { flex: 1, position: 'relative', minHeight: 0, overflow: 'hidden' },
  dragGrip: {
    padding: '0 10px',
    color: '#5555aa',
    cursor: 'row-resize',
...
const panelStyles: Record<string, React.CSSProperties> = {
  ...
  portId:   { width: 60, color: '#00d2ff', flexShrink: 0 },
  portName: { flex: 1, color: '#aaaacc' },
  portType: { fontSize: 11, flexShrink: 0 },
};
