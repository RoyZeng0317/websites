...
class SubjectCfg {
  ...
  const SubjectCfg({
    required this.name,
    required this.files,
    required this.wiNames,
    required this.wiRanges,
    required this.wiCount,
  });
}
...
const subjectConfig = {
  ...
  'common': SubjectCfg(
    name: '共同科目',
    files: [
      'assets/sql/90006_Common.sql',
      'assets/sql/90007_Common.sql',
      'assets/sql/90008_Common.sql',
      'assets/sql/90009_Common.sql',
    ],
    wiNames: {
      1: '90006 職業安全衛生',
      2: '90007 工作倫理與職業道德',
    ...
    wiRanges: [
      {'s': 9000601, 'e': 90006100},
      {'s': 9000701, 'e': 90007100},
      {'s': 9000801, 'e': 90008100},
      {'s': 9000901, 'e': 90009100},
    ],
    wiCount: 4,
  ),
};
...
List<String> splitOptions(String str) {
  ...
  for (final ch in str.runes) {
    if (ch == 0x28 || ch == 0xFF08) {
      depth++;
    } else if (ch == 0x29 || ch == 0xFF09) {
      depth--;
    }
    if (ch == 0x2C && depth == 0) {
...
List<Question> parseSQL(String t) {
  final r7 = RegExp(
    r"VALUES\s*\(\s*(\d+)\s*,\s*'([^']+)'\s*,\s*'([^']+)'\s*,\s*'([^']+)'\s*,\s*'([^']+)'\s*,\s*'([^']+)'\s*,\s*'([^']*)'\s*\)",
  );
  final r6 = RegExp(
    r"VALUES\s*\(\s*(\d+)\s*,\s*'([^']+)'\s*,\s*'([^']+)'\s*,\s*'([^']+)'\s*,\s*'([^']+)'\s*,\s*'([^']*)'\s*\)",
  );
  final r5 = RegExp(
    r"VALUES\s*\(\s*(\d+)\s*,\s*'([^']+)'\s*,\s*'([^']+)'\s*,\s*'([^']+)'\s*,\s*'([^']+)'\s*\)",
  );
  ...
  for (final line in t.split('\n')) {
    ...
    var c = 7;
    if (m == null) {
      m = r6.firstMatch(line);
      c = 6;
    }
    if (m == null) {
      m = r5.firstMatch(line);
      c = 5;
    }
    if (m == null) continue;
...
String getQuestionImageUrl(Question q, int wi, String subjectId) {
  ...
  if (q.image != null) {
    final img = q.image!.replaceFirst('../../', '');
...
const categoryItems = [
  ...
  {
    'label': '電機機械',
    'items': ['電機職類', '金屬及機械加工職類', '機械及設備修護職類'],
  },
  {
    'label': '勞安營造',
    'items': ['勞工安全衛生操作職類', '營造職類'],
  },
  {
    'label': '美容餐飲',
    'items': ['美容美髮服飾職類', '餐飲食品相關職類'],
  },
  {
    'label': '商業農業',
    'items': ['商業服務印刷製版職類', '農業職類'],
  },
  {
    'label': '化工配管',
    'items': ['化工職類', '焊接配管職類'],
...
class _LandingPageState extends State<LandingPage> {
  ...
  void _onSubItemTap(String subItem) {
    if (subItem == '資訊職類')
      setState(() => _showComputerPanel = !_showComputerPanel);
    if (subItem == '電子儀表種類')
      setState(() => _showElectronicPanel = !_showElectronicPanel);
  }
  ...
  void _openQuiz(String subjectId) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => QuizPage(subjectId: subjectId)),
    );
  }
  ...
  Widget _buildTitle(BuildContext c) {
    return Padding(
      padding: EdgeInsets.symmetric(
        vertical: respSize(c, 30),
        horizontal: respSize(c, 16),
      ),
      child: Text(
        '即測即評題庫網',
        style: TextStyle(
          fontSize: respFont(c, 28),
  ...
  Widget _buildMenu(BuildContext c) {
    ...
    return Container(
      ...
      padding: EdgeInsets.symmetric(
        vertical: respSize(c, 12),
        horizontal: respSize(c, 8),
      ),
      ...
      child: w < 480
          ? Column(
              children: categoryItems
                  .map((cat) => _buildMenuItem(c, cat))
                  .toList(),
            )
          : Wrap(
              spacing: respSize(c, 8),
              runSpacing: respSize(c, 8),
      ...
      itemBuilder: (_) => items.isEmpty
          ...
          : items
                .map(
                  (i) => PopupMenuItem(
                    value: i,
                    child: Text(i, style: TextStyle(fontSize: respFont(c, 13))),
                  ),
                )
                .toList(),
      child: Container(
        padding: EdgeInsets.symmetric(
          horizontal: respSize(c, 14),
          vertical: respSize(c, 8),
        ),
        decoration: BoxDecoration(
  ...
  Widget _buildPanelSection(BuildContext c) {
    return Padding(
      padding: EdgeInsets.symmetric(
        horizontal: respSize(c, 12),
        vertical: respSize(c, 8),
      ),
      child: Column(
        children: [
          if (_showComputerPanel)
            _buildPanel(c, '丙級題庫', [
              _examLink(c, '電腦軟體應用', () => _openQuiz('11800')),
              _examLink(c, '電腦軟體設計', () {}),
              _examLink(c, '電腦硬體裝修', () {}),
            ]),
          if (_showElectronicPanel)
            _buildPanel(c, '丙級題庫', [_examLink(c, '工業電子丙級', () {})]),
          if (_showBClass)
            _buildPanel(c, '乙級題庫', [
  ...
  Widget _examLink(BuildContext c, String label, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        padding: EdgeInsets.symmetric(
          horizontal: respSize(c, 18),
          vertical: respSize(c, 10),
        ),
        decoration: BoxDecoration(
  ...
  Widget _buildContact(BuildContext c) {
    return Column(
      children: [
        ...
        GestureDetector(
          onTap: () =>
              launchUrl(Uri.parse('mailto:你的信箱@gmail.com?subject=即測即評題庫網建議')),
          child: Text(
            '[聯絡信箱]',
            style: TextStyle(
              fontSize: respFont(c, 13),
  ...
  Widget _buildFooter(BuildContext c) {
    return Container(
      ...
      child: Text(
        '© 2026 即測即評題庫網. All rights reserved.',
        textAlign: TextAlign.center,
        style: TextStyle(
          fontSize: respFont(c, 11),
          color: const Color(0xFF94A3B8),
        ),
      ),
    );
...
class _QuizPageState extends State<QuizPage> {
  ...
  void initState() {
    super.initState();
    _currentSubject = widget.subjectId;
    _loadSubject();
  }
  ...
  void _toggleMode() {
    setState(() {
      ...
      _currentIndex = 0;
      if (_examMode) {
        _examQuestions = _shuffle(
          _allQuestions,
        ).take(min(80, _allQuestions.length)).toList();
        _answeredMap = {};
        _submittedMap = {};
      } else {
        _examQuestions = [];
  ...
  void _toggleOpt(int qId, int idx, String type) {
    ...
    setState(() {
      ...
      if (type == 'single') {
        _answeredMap[qId] = [idx];
      } else {
        final p = _answeredMap[qId]!.indexOf(idx);
        p >= 0 ? _answeredMap[qId]!.removeAt(p) : _answeredMap[qId]!.add(idx);
      }
    });
  }

  void _submitAnswer(int qId) {
    final sel = _answeredMap[qId];
    if (sel == null || sel.isEmpty) return;
    setState(() => _submittedMap[qId] = true);
  }
  ...
  void _submitExam() {
    final score = _examQuestions
        .where((qq) => _listEq(_answeredMap[qq.id] ?? [], qq.answer))
        .length;
  ...
  void _goToExamQuestion(int idx) {
    if (idx >= 0 && idx < _examQuestions.length)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          _examMode ? '考試模式' : '${_cfg.name} - 逐題練習',
          style: TextStyle(fontSize: respFont(context, 15)),
        ),
        backgroundColor: const Color(0xFF1E1E1E),
        leading: IconButton(
          ...
          if (!_isLoading)
            TextButton(
              ...
              child: Text(
                _examMode ? '練習模式' : '考試模式',
                style: TextStyle(
                  color: _examMode ? Colors.redAccent : Colors.grey,
                  fontSize: respFont(context, 12),
                ),
              ),
            ),
        ],
            ...
            child: Container(
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<int>(
                  ...
                  style: TextStyle(
                    fontSize: respFont(context, 11),
                    color: const Color(0xFFE0E0E0),
                  ),
                  items: [
                    const DropdownMenuItem(
                      value: 0,
                      child: Text('全部工作項目', style: TextStyle(fontSize: 11)),
                    ),
                    ..._cfg.wiNames.entries.map(
                      (e) => DropdownMenuItem(
                        value: e.key,
                        child: Text(
                          e.value,
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  ...
                  items: [
                    const DropdownMenuItem(
                      value: 'all',
                      child: Text('全部題型', style: TextStyle(fontSize: 11)),
                    ),
                    const DropdownMenuItem(
                      value: 'single',
                      child: Text('單選題', style: TextStyle(fontSize: 11)),
                    ),
                    if (_currentSubject != 'common')
                      const DropdownMenuItem(
                        value: 'multiple',
                        child: Text('複選題', style: TextStyle(fontSize: 11)),
                      ),
                  ],
                  onChanged: (v) {
                    if (v != null) {
          ...
          Wrap(
            ...
            children: List.generate(_examQuestions.length, (i) {
              ...
              if (_examSubmitted) {
                ...
                bg = hasAns ? const Color(0xFF1A3A5C) : const Color(0xFF252525);
              }
              return GestureDetector(
  ...
  Widget _buildContent() {
    ...
    if (_filteredQuestions.isEmpty)
      return const Center(
        child: Text('無符合條件的題目', style: TextStyle(color: Color(0xFF888888))),
      );
  ...
  Widget _buildExamContent() {
    if (_examQuestions.isEmpty)
      return const Center(
        child: Text('無題目', style: TextStyle(color: Color(0xFF888888))),
      );
    final q = _examQuestions[_currentIndex];
    final answeredCount = _examQuestions
        .where((qq) => (_answeredMap[qq.id] ?? []).isNotEmpty)
        .length;
    return Column(
      children: [
        Padding(
          ...
          child: Row(
            ...
            children: [
              Text(
                '作答: $answeredCount / ${_examQuestions.length}',
                style: TextStyle(
                  color: const Color(0xFF888888),
                  fontSize: respFont(context, 12),
                ),
        ...
        if (_examSubmitted) ...[
          const SizedBox(height: 4),
          Text(
            '得分: ${(_examQuestions.where((qq) => _listEq(_answeredMap[qq.id] ?? [], qq.answer)).length / _examQuestions.length * 100).round()}分',
            style: TextStyle(
              color: const Color(0xFF4CAF50),
              fontWeight: FontWeight.bold,
              fontSize: respFont(context, 13),
            ),
          ),
        ],
        Expanded(
        ...
        if (_examSubmitted)
          Padding(
            ...
            child: TextButton(
              onPressed: () =>
                  setState(() => _examOverviewVisible = !_examOverviewVisible),
  ...
  Widget _buildQuestionContent({
    ...
    required Function(int, int, String) onToggleOpt,
            ...
            child: Column(
              ...
              children: [
                ...
                if (question.image != null ||
                    RegExp(r'^\d+\.\s*\(').hasMatch(question.question))
                ...
                if (question.image != null ||
                  ...
                  SizedBox(
                    ...
                    child: ClipRRect(
                      borderRadius: BorderRadius.circular(6),
                      child: Image.network(
                        getQuestionImageUrl(question, wi, _currentSubject),
                        width: double.infinity,
                        height: 300,
                        fit: BoxFit.contain,
                ...
                ...List.generate(question.options.length, (i) {
                  ...
                  return GestureDetector(
                    onTap: done
                        ? null
                        : () => onToggleOpt(question.id, i, question.type),
                    child: Container(
                      width: double.infinity,
                ...
                if (showSubmit && !done)
                  Padding(
                    ...
                    child: SizedBox(
                      width: double.infinity,
                      child: ElevatedButton(
                        onPressed: sel.isEmpty ? null : onSubmit,
                ...
                if (done) ...[
                  ...
                  Container(
                    ...
                    padding: const EdgeInsets.all(12),
                    decoration: BoxDecoration(
                      color: _listEq(sel, question.answer)
                          ? const Color(0xFF1A3A1A)
                          : const Color(0xFF3A1A1A),
                      borderRadius: BorderRadius.circular(6),
                      border: Border.all(
          ...
          if (isExam && !_examSubmitted) ...[
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: ElevatedButton(
                    onPressed: _currentIndex < _examQuestions.length - 1
                        ? () => setState(() => _currentIndex++)
                        : null,
                    child: const Text('下一題 →'),
                  ),
                ...
                SizedBox(
                  ...
                  child: TextField(
                    ...
                    onSubmitted: (v) {
                      ...
                      if (n != null && n >= 1 && n <= _filteredQuestions.length)
  ...
  Widget _buildQuestionText(Question q, int wi) {
    final replaced = q.question.replaceAllMapped(
      RegExp(r'([a-zA-Z])\s*圖'),
      (m) => '${m.group(1)!}圖',
    );
    return Text(
