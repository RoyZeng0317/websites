...
async function runUpdateTask() {
  // sudo -n: non-interactive, fails immediately if password required
  // Fix on Pi: echo "$(whoami) ALL=(ALL) NOPASSWD: /usr/bin/apt-get" | sudo tee /etc/sudoers.d/vaultix-apt && sudo chmod 440 /etc/sudoers.d/vaultix-apt
  const { stdout, stderr } = await execAsync(
    'sudo -n apt-get update -qq && sudo -n apt-get upgrade -y 2>&1',
    { timeout: 300_000 }
  )
  return (stdout + stderr).slice(-1000) || '更新完成'
}
...
async function runTask(task) {
  ...
  try {
    ...
    else if (task.type === 'update') message = await runUpdateTask()
  } catch (err) {
    status  = 'error'
