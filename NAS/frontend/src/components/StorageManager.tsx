...
export default function StorageManager({ onClose }: Props) {
  ...
  async function mount(partName: string, existingMountpoint?: string | null) {
    ...
    if (!folderName) { setMsg('請輸入資料夾名稱'); return }
    setBusy(partName)
    setMsg('')
    try {
      ...
      setMsg(`✓ 已在 NAS 中掛載 /dev/${partName}，重新整理檔案列表即可看到`)
      await load()
    } catch (e: unknown) {
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-lg flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-700 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-semibold">磁碟管理 / Storage Manager</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {loading && (
            <div className="text-center text-gray-400 py-8">
              ...
              <p className="text-sm">偵測裝置中…</p>
            </div>
          )}

          {error && !loading && (
            <div className="bg-red-900/30 border border-red-700 rounded-lg px-4 py-3 text-red-300 text-sm">
              <p className="font-medium mb-1">無法讀取磁碟資訊</p>
          ...
          {!loading && !error && mountableDisks.length === 0 && (
            <div className="text-center text-gray-500 py-8 text-sm space-y-1">
              <p>未偵測到外接裝置</p>
              <p className="text-xs text-gray-600">請確認裝置已連接到樹莓派</p>
              <p className="text-xs text-gray-600">
                系統磁碟（{SYSTEM_PREFIXES.join(', ')} 開頭）不在列表中
              </p>
            </div>
          )}
            ...
            return (
              <div key={disk.name} className="bg-gray-800 rounded-lg overflow-hidden">
                ...
                <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-700">
                  ...
                  <span className="ml-auto text-xs bg-blue-900/40 text-blue-400 px-2 py-0.5 rounded">外接</span>
                </div>
                  ...
                  return (
                    <div key={part.name} className="px-4 py-3 border-t border-gray-700/50 space-y-2">
                      ...
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {/* Status badge */}
                        {sysMount
                          ? <span className="shrink-0 text-xs bg-yellow-900/40 text-yellow-400 px-2 py-0.5 rounded">已掛載</span>
                          : <span className="shrink-0 text-xs bg-gray-700 text-gray-400 px-2 py-0.5 rounded">未掛載</span>
                        }
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {busy === part.name ? '…' : sysMount ? '加入 NAS' : '掛載至 NAS'}
                        </button>
                      </div>
                      {sysMount && (
                        <p className="text-gray-600 text-xs">
                          已被系統掛載，點「加入 NAS」可在 NAS 檔案列表中存取
                        </p>
                      )}
                    </div>
                  )
          ...
          <div className="bg-gray-800/50 border border-gray-700 rounded-lg px-4 py-3 text-xs text-gray-500 space-y-1.5">
            <p className="text-gray-400 font-medium">若掛載失敗，請在樹莓派確認 sudo 權限：</p>
