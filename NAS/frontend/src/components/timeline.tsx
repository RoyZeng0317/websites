  ...
  return (
    <div className="min-h-dvh flex flex-col bg-gray-900 text-white">
      <header className="flex items-center justify-between px-6 py-4 border-b border-gray-700 shrink-0">
        ...
        <h1 className="text-xl font-bold text-orange-400">Media Timeline</h1>
      ...
      <main className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-500 gap-3">
            ...
            <p className="text-xs text-gray-600">Upload photos or videos to view them here</p>
          </div>
        ) : (
          <div className="max-w-5xl mx-auto px-4 py-6 space-y-8">
            <p className="text-sm text-gray-400 px-1">{totalItems} media files</p>
            {groups.map(g => (
              <section key={g.date}>
                <h2 className="text-lg font-semibold text-orange-400 mb-3 px-1 sticky top-0 bg-gray-900/90 backdrop-blur-sm py-2 z-10">
                  {dateLabel(g.date)}
                  <span className="text-sm font-normal text-gray-500 ml-2">{g.items.length} items</span>
                </h2>
