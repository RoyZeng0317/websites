...
export default function AutomaticPanel({ onClose, onOpenLogs }: Props) {
  ...
  async function load() {
    ...
    try {
      ...
      if (ut) setUCron(ut.cron_expr)
    } catch { toast.error('無法載入排程') }
  ...
  async function addTask() {
    if (!fName.trim()) return toast.error('請填寫任務名稱')
    if (!fSrc.trim() || !fDst.trim()) return toast.error('請填寫來源與目標路徑')
    if (!fCron) return toast.error('請設定排程')
    setSaving(true)
    try {
  ...
  async function saveUpdate(enabled?: boolean) {
    if (!uCron) return toast.error('請設定排程')
    setUSaving(true)
    try {
      if (updateTask) {
        await apiJson(`/api/schedule/tasks/${updateTask.id}`, {
          method: 'PUT', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ cron_expr: uCron, ...(enabled !== undefined && { enabled }) }),
        })
      } else {
        await apiJson('/api/schedule/tasks', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: '系統自動更新', type: 'update', cron_expr: uCron, enabled: enabled !== false }),
        })
      }
      toast.success('設定已儲存')
      await load()
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-700 shrink-0">
          <div className="flex items-center gap-3">
            <h2 className="text-white font-semibold">排程管理</h2>
            {onOpenLogs && (
              <button
                onClick={onOpenLogs}
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-3">
          <p className="text-sm font-medium text-gray-300">新增備份任務</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-gray-500 mb-1">任務名稱</label>
              <input
                type="text"
                value={fName}
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">保留份數</label>
              <input
                type="number"
                value={fKeep}
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">來源路徑</label>
              <input
                type="text"
                value={fSrc}
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">目標路徑</label>
              <input
                type="text"
                value={fDst}
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1">排程</label>
      ...
      {/* Task list */}
      {tasks.length === 0 ? (
        <div className="text-center py-16 text-gray-600 text-sm">尚無備份任務</div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1.5 text-xs text-gray-500 space-y-0.5">
                    ...
                    <div>{cronLabel(t.cron_expr)} ／ 保留 {t.keep_count} 份</div>
                    <div className="flex items-center gap-2 pt-0.5">
                      <span>上次執行：{fmtTime(t.last_run)}</span>
...
function UpdateTab({ task, uCron, setUCron, saving, runningId, onSave, onRun, onToggle }: UpdateTabProps) {
  ...
  return (
    <div className="max-w-lg space-y-4">
      ...
      <div className="bg-blue-900/20 border border-blue-800/40 rounded-xl p-4 text-sm text-blue-300">
        執行 <code className="font-mono bg-blue-900/40 px-1 rounded">sudo apt-get update &amp;&amp; apt-get upgrade -y</code>，需要 sudo NOPASSWD 權限。
      </div>
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-4">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="text-white text-sm font-medium">系統自動更新</p>
            {task ? (
              <p className="text-xs text-gray-500 mt-0.5 flex items-center gap-1.5">
                上次執行：{fmtTime(task.last_run)}
                <StatusBadge status={task.last_status} />
              </p>
            ) : (
              <p className="text-xs text-gray-600 mt-0.5">尚未建立排程</p>
            )}
          </div>
        ...
        {/* Schedule */}
        <div>
          <label className="block text-xs text-gray-500 mb-1.5">執行排程</label>
        ...
        <div className="flex gap-2">
          ...
          >
            {runningId === task?.id
              ? <><span className="animate-spin leading-none">◌</span><span>執行中</span></>
              : <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.347a1.125 1.125 0 010 1.972l-11.54 6.347a1.125 1.125 0 01-1.667-.986V5.653z" /></svg><span>立即執行</span></>
            }
          </button>
        </div>
      </div>
