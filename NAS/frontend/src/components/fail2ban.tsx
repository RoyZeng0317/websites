...
export default function Fail2ban() {
  ...
  async function unbanIp(jail: string, ip: string) {
    ...
    try {
      ...
      await loadStatus()
    } catch (err) { toast.error((err as { message?: string }).message ?? 'Unban failed') }
    setUnbanning(null)
  }
        ...
        <main className="flex-1 overflow-y-auto p-6 max-w-3xl mx-auto w-full space-y-4">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-gray-500 gap-4">
              ...
              <p className="text-sm">Fail2ban not installed or not running</p>
              <div className="bg-gray-800 rounded-xl border border-gray-700 p-4 max-w-md text-xs text-gray-500 space-y-1 font-mono">
                <p className="text-gray-400 font-sans text-xs font-semibold mb-2">Installation:</p>
                <p>sudo apt install fail2ban</p>
                <p className="text-gray-600 mt-2"># Allow NAS backend passwordless access:</p>
            ...
            <div className="flex flex-col items-center justify-center py-20 text-gray-500 gap-3">
              ...
              <p className="text-sm">No active jail rules</p>
            </div>
          ) : (
            <>
              {/* Summary */}
              <div className="grid grid-cols-3 gap-3">
                {[
                  { label: 'Jails', value: jails.length, color: 'text-orange-400' },
                  { label: 'Currently Banned', value: totalBanned, color: 'text-red-400' },
                  { label: 'Total Banned', value: jails.reduce((s, j) => s + j.totalBanned, 0), color: 'text-gray-300' },
                ].map(stat => (
                  ...
                  <div key={j.name} className="bg-gray-800 rounded-xl border border-gray-700/50 overflow-hidden">
                    <div className="p-4 space-y-3">
                      <div className="flex items-center gap-2">
                        ...
                        }`}>{j.status === 'active' ? 'Active' : j.status}</span>
                      </div>

                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                        {[
                          { label: 'Currently Banned', value: j.banned,      color: 'text-red-400' },
                          { label: 'Total Banned',     value: j.totalBanned, color: 'text-gray-300' },
                          { label: 'Current Failures', value: j.failed,      color: 'text-yellow-400' },
                          { label: 'Total Failures',   value: j.totalFailed, color: 'text-gray-300' },
                        ].map(s => (
                      ...
                      {j.bannedIps.length > 0 && (
                        <div>
                          <p className="text-xs text-gray-500 mb-1.5">Banned IPs</p>
          ...
          <div className="shrink-0 flex items-center gap-2 px-6 py-3 border-b border-gray-700/50 bg-gray-800/40 flex-wrap">
            ...
            <div className="flex bg-gray-900 border border-gray-700 rounded-lg p-0.5 text-xs gap-0.5">
              {([
                ...
                { key: 'ban',   label: 'Ban',   color: 'data-[active]:bg-red-600' },
                { key: 'unban', label: 'Unban', color: 'data-[active]:bg-green-600' },
                { key: 'found', label: 'Found', color: 'data-[active]:bg-yellow-600' },
              ] as const).map(f => (
                <button
                  key={f.key}
            ...
            <span className="text-xs text-gray-600 ml-auto">
              {logLoading ? 'Loading…' :
                logRaw ? `${filteredRaw.length} lines` : `${filteredLog.length} events`}
            </span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {logLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-gray-600 gap-2">
                ...
                <p className="text-sm">
                  {logLines.length === 0 ? 'Cannot read log file' : 'No records match the filter'}
                </p>
                {logLines.length === 0 && (
                  <p className="text-xs text-gray-700 text-center max-w-xs">
                    Please check that <code className="text-gray-600">/var/log/fail2ban.log</code> exists,<br />
                    or the backend user has sudo read permissions.
                  </p>
                )}
              </div>
            ) : (
