...
export default function Locker({ onClose }: Props) {
  ...
  function startMobilePoll() {
    ...
    cdTimerRef.current = setInterval(() => {
      setMobileCountdown(n => {
        if (n <= 1) { stopMobilePoll(); setAuthError('Timeout, please try again'); return 0 }
        return n - 1
      })
    }, 1000)
  ...
  // ── Fix: reset stuck state when returning from background ──
  useEffect(() => {
    let hiddenAt = 0
    function onVisibility() {
      if (document.visibilityState === 'hidden') {
        ...
        // Force-clear busy state (Promise may be hanging)
        setAuthBusy(false)
        setAuthError('')
        // Idle over 5 minutes: auto-lock (security)
        if (hiddenAt && Date.now() - hiddenAt > AUTO_LOCK_MS) {
          setScreen(s => s === 'unlocked' ? 'locked' : s)
        }
      }
    }
  ...
  function checkWebAuthnReady(): boolean {
    if (!window.isSecureContext) {
      setAuthError('Fingerprint requires HTTPS.\nPlease access via https://casaos-nas.web.app')
      return false
    }
    if (!browserSupportsWebAuthn()) {
      setAuthError('This browser does not support biometrics. Use Chrome, Safari, or Edge.')
      return false
    }
    return true
  }
  ...
  function parseWebAuthnError(e: unknown): string {
    ...
    if (
      ...
      return `Fingerprint error: URL mismatch\nPlease access via https://casaos-nas.web.app\n(${msg})`
    }
    return msg
  }
  ...
  async function handleRegister() {
    ...
    try {
      ...
      setHasCred(true)
      toast.success('Fingerprint set up, auto-unlocked')
      setScreen('unlocked')
      apiJson<{ registered: boolean; devices: Device[] }>('/api/locker/webauthn/status')
  ...
  async function handleAuth(retried = false) {
    if (!checkWebAuthnReady()) return
    setAuthBusy(true); setAuthError('')
    try {
      const originParam = `?origin=${encodeURIComponent(window.location.origin)}`
      ...
      const err = e as Error
      const msg = err.message ?? ''
      // Challenge expired due to backend restart → auto-retry once
      if (!retried && (msg.includes('Challenge expired') || msg.includes('驗證已過期') || msg.includes('expired'))) {
        setAuthBusy(false)
        setTimeout(() => handleAuth(true), 300)
        return
      }
      if (msg === 'no_credential') {
        setHasCred(false)
        setAuthError('No fingerprint set up. Please set one up first.')
      } else {
        setAuthError(parseWebAuthnError(e))
      }
    }
  ...
  async function handleTotpUnlock() {
    ...
    if (clean.length !== 6) { setAuthError('Please enter a 6-digit code'); return }
    setAuthBusy(true); setAuthError('')
    try {
  ...
  async function handleDeleteDevice(credId: number) {
    if (!confirm('Remove fingerprint authentication for this device?')) return
    try {
  ...
  async function handleUpload(files: FileList) {
    ...
    setUploading(false)
    if (ok)   toast.success(`${ok} files uploaded`)
  ...
  function applyPath() {
    ...
    if (!p) { toast.error('Path cannot be empty'); return }
    setLockerPath(p)
    savePrefs({ locker_path: p }) // save to sda1
    toast.success('Applied'); setScreen('unlocked')
  }
      ...
      <div className="fixed inset-0 z-50 bg-gray-950 flex flex-col items-center justify-center gap-8">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="text-center">
            <p className="text-white font-bold text-lg">Private Album</p>
            <p className="text-gray-600 text-sm mt-0.5">
              {lockMode === 'fingerprint' ? 'Fingerprint / Face ID Unlock' : 'Enter App Verification Code'}
            </p>
          </div>
        </div>
          ...
          {lockMode === 'fingerprint' ? (
              ...
              {hasCredential ? (
                ...
                >
                  {authBusy
                    ...
                    : <><FingerprintIcon className="w-6 h-6"/><span>Unlock with Fingerprint</span></>
                  }
                </button>
              ) : (
                <button
                ...
                >
                  {authBusy
                    ? <><span className="w-5 h-5 border-2 border-white/40 border-t-white rounded-full animate-spin"/><span>Setting up…</span></>
                    : <><FingerprintIcon className="w-6 h-6"/><span>Set Up Fingerprint</span></>
                  }
                </button>
              )}
              {hasCredential && (
                <button onClick={handleRegister} disabled={authBusy}
                  ...
                  Use another device / Re-setup fingerprint
                </button>
              )}
            </>
          ) : (
            <>
              <div className="w-full space-y-3">
                <p className="text-xs text-gray-500 text-center">Open the NAS Auth App on your phone and copy the Private Album code</p>
                <input
                  type="text"
                  inputMode="numeric"
                  maxLength={7}
            ...
            <div className="w-full space-y-4 text-center">
              {mobileCountdown > 0 ? (
                <>
                  ...
                  <div>
                    <p className="text-white font-medium">Waiting for mobile biometrics</p>
                    <p className="text-gray-500 text-sm mt-1">
                      Open NAS Auth App on your phone, tap <span className="text-orange-400 font-medium">"Unlock Private Album"</span>
                    </p>
                  </div>
                  <p className="text-gray-700 text-xs tabular-nums">Waiting… {mobileCountdown}s</p>
                  <button
    ...
    return (
      <div className="fixed inset-0 z-50 bg-gray-950 flex flex-col">
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-800">
          ...
          <h1 className="text-white font-semibold">Private Album Settings</h1>
        </div>
        <div className="flex-1 overflow-y-auto p-5 space-y-5 max-w-lg mx-auto w-full">
          ...
          <section className="bg-gray-900 border border-gray-800 rounded-2xl p-4 space-y-3">
            <h2 className="text-sm font-medium text-gray-300">Private Folder Path</h2>
            <div className="flex gap-2">
              ...
              <button onClick={applyPath} className="px-4 py-2 rounded-xl bg-orange-600 hover:bg-orange-500 text-white text-sm font-medium transition-colors">Apply</button>
            </div>
            <p className="text-xs text-gray-600">Current: <span className="text-gray-400 font-mono">{lockerPath}</span></p>
          </section>

          {/* Registered devices */}
          <section className="bg-gray-900 border border-gray-800 rounded-2xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-medium text-gray-300">Registered Fingerprint Devices</h2>
              <button onClick={handleRegister} disabled={authBusy}
            ...
            {devices.length === 0 ? (
              <p className="text-xs text-gray-600">No devices registered</p>
            ) : (
                ...
                {devices.map(d => (
                  <div key={d.id} className="flex items-center justify-between bg-gray-800 rounded-xl px-3 py-2.5">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <p className="text-sm text-white">{d.device_name || 'Unknown device'}</p>
          ...
          <section className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
            <h2 className="text-sm font-medium text-gray-300 mb-1">Auto Lock</h2>
            <p className="text-xs text-gray-600">Auto lock after 5 minutes of inactivity</p>
          </section>
        </div>
      </div>
    )
      ...
      <div className="fixed inset-0 z-50 bg-gray-950 flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-800 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-semibold text-sm leading-tight">Private Album</p>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => { setPathDraft(lockerPath); setAuthError(''); setScreen('settings') }} title="Settings"
              className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-white hover:bg-gray-800 transition-colors">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-5 text-center">
              ...
              <div>
                <p className="text-gray-400 font-medium">Private Album is empty</p>
                <p className="text-gray-600 text-sm mt-1">Upload photos or videos — only you can see them</p>
              </div>
