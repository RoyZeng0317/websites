...
function AddDeviceModal({
  ...
  function submit() {
    ...
    if (!ip) { setIpErr('請輸入 IP 位址'); return }
    // basic IP / hostname validation
    const ipRe = /^(\d{1,3}\.){3}\d{1,3}$|^[a-zA-Z0-9]([a-zA-Z0-9\-]*\.?)+[a-zA-Z0-9]$/
    if (!ipRe.test(ip)) { setIpErr('格式錯誤，請輸入有效 IP 或主機名稱'); return }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4"
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl w-full max-w-sm"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800">
          <p className="font-semibold text-white">{initial ? '編輯設備' : '新增設備'}</p>
        ...
        <div className="p-5 space-y-4">
          {/* IP */}
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">IP 位址 / 主機名稱 <span className="text-red-400">*</span></label>
            <input
              value={form.ip}
          ...
          {/* Name */}
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">設備名稱（選填）</label>
            <input
              value={form.name}
              onChange={e => set('name', e.target.value)}
              placeholder="我的 NAS、Pi Server…"
          ...
          {/* Port */}
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">主要連接埠（選填）</label>
            <input
              value={form.port}
          ...
          {/* Note */}
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">備註（選填）</label>
            <input
              value={form.note}
          ...
          {confirmDel ? (
            <>
              <button onClick={onDelete}
                className="text-xs text-red-400 hover:text-red-300 transition-colors px-1.5 py-1 rounded">確認</button>
              <button onClick={() => setConfirmDel(false)}
                className="text-xs text-gray-500 hover:text-white transition-colors px-1.5 py-1 rounded">取消</button>
            </>
          ) : (
...
export default function System() {
  ...
  async function toggle(svcKey: keyof Services, apiName: string, active: boolean) {
    ...
    try {
      ...
      setErr(`操作失敗。請確認樹莓派上已安裝 ${apiName} 並設定 sudoers。`)
    } finally {
      setToggling(null)
    }
  }
  ...
  const services = [
    {
      ...
      controllable: true,
      connections: [
        { label: 'FTP 連線位址', value: `ftp://${ip}` },
        { label: 'FileZilla 設定', value: `主機: ${ip}  連接埠: 21  協定: FTP` },
      ],
      note: '需先執行 setup-services.sh；使用 NAS 帳號密碼登入',
    },
    {
      key: 'sftp' as const,
      apiName: '',
      ...
      connections: [
        ...
        { label: 'FileZilla 設定', value: `主機: sftp://${ip}  連接埠: 22` },
      ],
      note: '透過 SSH 連線，預設已啟用；使用樹莓派系統帳號（roy）',
    },
    {
      key: 'samba' as const,
      apiName: 'smbd',
      ...
      controllable: true,
      connections: [
        { label: 'Windows 掛載', value: `\\\\${ip}` },
        { label: 'macOS / Linux', value: `smb://${ip}` },
      ],
      note: '需先執行 setup-services.sh；Windows：檔案總管→「映射網路磁碟機」',
    },
    {
      key: 'webdav' as const,
      apiName: '',
      ...
      connections: [
        ...
        { label: 'Windows 掛載', value: `http://${ip}:3000/webdav` },
      ],
      note: '已內建於 NAS 後端；使用 NAS 帳號密碼登入。Windows：「新增網路位置」→輸入 URL',
    },
  ]
  ...
  return (
    <div className="min-h-dvh flex flex-col bg-gray-900 text-white">
      <header className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
        ...
        <h1 className="text-xl font-bold text-orange-400">系統設定</h1>
        <button onClick={refresh} className="text-sm text-gray-400 hover:text-white transition-colors">重新整理</button>
      </header>
      ...
      <main className="flex-1 p-6 max-w-2xl mx-auto w-full space-y-5">
        ...
        <section className="bg-gray-800 rounded-xl p-5 space-y-4">
          <h2 className="font-semibold text-orange-400">系統資訊</h2>
          {info ? (
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><p className="text-xs text-gray-400 mb-0.5">主機名稱</p><p className="font-mono">{info.hostname}</p></div>
              <div><p className="text-xs text-gray-400 mb-0.5">IP 位址</p><p className="font-mono">{info.ip}</p></div>
              <div><p className="text-xs text-gray-400 mb-0.5">持續運行</p><p>{uptime(info.uptime)}</p></div>
              <div>
                <p className="text-xs text-gray-400 mb-0.5">記憶體</p>
            ...
            <p className="text-sm text-gray-500 animate-pulse">載入中…</p>
          )}
        </section>
        ...
        {info?.disk && (
          <section className="bg-gray-800 rounded-xl p-5 space-y-3">
            <h2 className="font-semibold text-orange-400">儲存空間</h2>
            <div className="flex justify-between text-sm mb-1">
              <span>{bytes(info.disk.used)} <span className="text-gray-400">已使用</span></span>
              <span className="text-gray-400">{bytes(info.disk.free)} 可用 / {bytes(info.disk.total)} 總計</span>
            </div>
            ...
            <p className="text-xs text-gray-500 text-right">{((info.disk.used / info.disk.total) * 100).toFixed(1)}% 已使用</p>
          </section>
        )}

        {/* Network Services */}
        <div>
          <h2 className="font-semibold text-orange-400 mb-3 px-1">網路服務</h2>
              ...
              <div key={svc.key} className="bg-gray-800 rounded-xl p-5 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {svc.controllable && (
                    ...
                    >
                      {toggling === svc.key ? '…' : svc.active ? '停止服務' : '啟動服務'}
                    </button>
                  )}
                </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3 px-1">
            <h2 className="font-semibold text-orange-400">已新增設備</h2>
            <button
          ...
          {devices.length === 0 ? (
            <div className="bg-gray-800/50 border border-dashed border-gray-700 rounded-xl p-8 text-center">
              ...
              <p className="text-sm text-gray-500">尚未新增任何設備</p>
              <p className="text-xs text-gray-600 mt-1">點擊右上角「新增設備」輸入 IP 位址即可添加</p>
            </div>
          ) : (
            <div className="space-y-3">
              {devices.map(d => (
