...
export default function SharePage() {
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND}/api/share/${token}`)
      ...
      .catch(() => { setStatus('error'); setErrMsg('Cannot connect to server') })
  }, [token])
  ...
  async function verify(tok: string, pwd: string) {
    try {
      ...
      setStatus('ready')
    } catch { setStatus('error'); setErrMsg('Verification failed') }
  }
  ...
  const permLabel = { view: 'View only', download: 'Download only', edit: 'Editable' }
    ...
    <div className="min-h-dvh flex items-center justify-center bg-gray-900 text-white p-6">
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center space-y-1">
          ...
          <p className="text-gray-400 text-sm">This share link is password protected</p>
    ...
    <div className="min-h-dvh flex flex-col bg-gray-900 text-white">
      <header className="px-6 py-4 border-b border-gray-700 flex items-center justify-between">
        <div className="min-w-0">
          ...
          <p className="text-xs text-gray-400">{info ? permLabel[info.permission] : ''}{info?.oneTime ? ' · One-time' : ''}</p>
        </div>
        ...
        <>
          ...
          <div className="flex items-center justify-between px-6 py-3 border-b border-gray-800">
            <nav className="flex items-center gap-1 text-sm text-gray-400 flex-wrap min-w-0">
              <button onClick={() => setPathSegments([])} className="hover:text-orange-400 transition-colors shrink-0">Root</button>
            ...
            {info.permission === 'edit' && (
              <div className="shrink-0 ml-4">
                <button onClick={() => fileInputRef.current?.click()} className="px-3 py-1.5 text-sm rounded-lg bg-orange-500 hover:bg-orange-600 text-white transition-colors">Upload</button>
          ...
          {uploadProgress !== null && (
            <div className="px-6 py-2 bg-gray-800 border-b border-gray-700">
              <div className="flex items-center gap-3 text-sm text-gray-300">
                <span className="shrink-0">Uploading {uploadProgress}%</span>
            ...
            {loadingFiles ? (
              ...
              <ul className="px-4 py-2">
                ...
                {items.length === 0 && !loadingFiles && (
                  <li className="text-center text-gray-600 text-sm py-16">This folder is empty</li>
                )}
              </ul>
            )}
          </main>
      ...
      {info && !info.isFolder && info.permission === 'view' && (
        <main className="flex-1 flex items-center justify-center text-gray-500 text-sm">
          View only — downloads not allowed
        </main>
      )}
    </div>
  )
