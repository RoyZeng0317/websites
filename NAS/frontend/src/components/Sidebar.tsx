...
export default function Sidebar({ currentPath, onNavigate, mountedDrives, onOpenStorage, isAdmin, onOpenVisualStudio, onOpenArduino, onOpenAnaconda, favorites = [], onOpenSnapshots }: Props) {
  const { t } = useLang()
  ...
  return (
    <aside className="w-44 shrink-0 bg-gray-900 border-r border-gray-800 flex flex-col overflow-y-auto">
      ...
      {favorites.length > 0 && (
        <div className="px-2 pt-2 pb-2 border-t border-gray-800">
          ...
          {favorites.map(fav => {
            ...
            const name = parts[parts.length - 1] ?? (fav || '根目錄')
      ...
      {/* Dev tools row */}
      {(codeServerUrl || onOpenVisualStudio || onOpenArduino || onOpenAnaconda) && (
        <div className="px-2 pb-3 border-t border-gray-800 pt-2">
          <p className="text-[10px] text-gray-600 uppercase tracking-widest mb-1.5 px-1 font-medium">開發工具</p>
          <div className="flex gap-1">
            ...
            {codeServerUrl && (
              <a
                ...
                title={t.openVSCode}
                className="flex-1 flex flex-col items-center gap-1 py-2 rounded-lg text-gray-400 hover:text-white hover:bg-gray-800 transition-colors"
              >
                ...
                <span className="text-[10px] leading-none">VSC</span>
              </a>
            )}
            {/* Visual Studio */}
            {onOpenVisualStudio && (
              ...
              >
                ...
                <span className="text-[10px] leading-none">VS</span>
              </button>
            )}
            {/* Arduino IDE */}
            {onOpenArduino && (
              <button
                ...
                className="flex-1 flex flex-col items-center gap-1 py-2 rounded-lg text-gray-400 hover:text-white hover:bg-gray-800 transition-colors"
              >
                <svg className="w-4 h-4 text-[#00878A]" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                ...
                <span className="text-[10px] leading-none">Arduino</span>
              </button>
            )}
            {/* Anaconda / Jupyter */}
            {onOpenAnaconda && (
              <button
                onClick={onOpenAnaconda}
              ...
              >
                ...
                <span className="text-[10px] leading-none">Jupyter</span>
              </button>
            )}
          </div>
        </div>
