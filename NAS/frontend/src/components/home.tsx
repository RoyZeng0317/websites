...
import { Form, useAsyncError, useNavigate } from 'react-router-dom'
...
import QuartusPanel from './QuartusPanel'
import KiCadPanel from './KiCadPanel'
...
export default function Home() {
  ...
  const [showQuartus, setShowQuartus] = useState(false)
  const [showKiCad, setShowKiCad]         = useState(false)
  const [showAndroidStudio, setShowAndroidStudio] = useState(false)
  ...
  async function toggleFavorite(path: string) {
    ...
    toast.success(isFavorite(path) ? 'Removed from favorites' : 'Added to favorites', { duration: 1500 })
  }

  // ── 快照 ────────────────────────────────────────────────
  async function createSnapshot(path: string) {
    const tid = toast.loading(`Creating snapshot: ${path.split('/').pop() ?? path}…`)
    try {
      ...
      toast.success('Snapshot created', { id: tid })
    } catch (err: unknown) {
      toast.error((err as { message?: string }).message ?? 'Snapshot failed', { id: tid })
    }
  }
  ...
  async function executeMove(intent: DropIntent) {
    ...
    try {
      ...
      toast.success(`Moved to ${intent.toName}`)
      await loadItems()
    } catch (e: unknown) {
  ...
  async function executeBulkMove(names: string[], toFolderPath: string, toFolderName: string) {
    ...
    if (ok > 0)   toast.success(`Moved ${ok} items to "${toFolderName}"`)
    if (fail > 0) toast.error(`${fail} items failed to move`)
    setSelectedItems(new Set())
    await loadItems()
  }
  ...
  async function executeCrossTabTransfer(action: 'move' | 'copy') {
    ...
    if (ok > 0) toast.success(`${verb} ${ok} items to "${xTo || 'Root'}"`)
    if (fail > 0) toast.error(`${fail} items failed to ${action === 'move' ? 'move' : 'copy'}`)
    await loadItems()
  }
  ...
  async function executeCopy(intent: DropIntent) {
    ...
    try {
      ...
      toast.success(`Copied as ${res.name}`)
      await loadItems()
    } catch (e: unknown) {
      toast.error((e as Error).message)
    }
  ...
  async function handleCreateQuickFolders(names: string[]) {
    ...
    for (const name of names) {
      try {
        ...
        // "already exists" comes back as an EEXIST / 400 error — treat as skip
        if (err?.message?.includes('exist') || err?.status === 409) {
          skipped++
        } else {
          fail++
        }
    ...
    if (ok > 0 || skipped > 0) {
      const msg = `${t.quickFoldersDone(ok + skipped)}${skipped > 0 ? ' ' + t.quickFoldersExist : ''}`
      toast.success(msg)
    }
    if (fail > 0) toast.error(t.quickFoldersFail)
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden relative z-10">
        ...
        <Sidebar
          ...
          onOpenCircuit={() => setShowCircuit(true)}
          onOpenQuartus={() => setShowQuartus(true)}
          onOpenKiCad={() => setShowKiCad(true)}
...
{/*            
          <button
            ...
            className="px-3 py-1.5 text-sm rounded-lg bg-gray-700 hover:bg-amber-600 text-white transition-colors max-[480px]:px-2 max-[480px]:py-1 max-[480px]:text-xs"
          >
            ⚡ {t.quickFolders}
          </button>
           */}
      ...
      {showCircuit && <Circuit onClose ={() => setShowCircuit(false)} />}
      {showQuartus && <QuartusPanel onClose={() => setShowQuartus(false)} />}
      {showKiCad && <KiCadPanel onClose={() => setShowKiCad(false)} />}
      {showAndroidStudio && <AndroidStudio onClose={() => setShowAndroidStudio(false)} />}
      {showOpenClaw && <OpenClawPanel onClose={() => setShowOpenClaw(false)} />}
      {showTodo && <TodoPanel onClose={() => setShowTodo(false)} />}
      {showFileConverter && <FileConverter onClose={() => setShowFileConverter(false)} />}
      {editVideoPath && (
        <EditVideo
          filePath={editVideoPath === '__browse__' ? null : editVideoPath}
          onClose={() => setEditVideoPath(null)}
          onDone={loadItems}
        />
      )}
      {showTrash && (
          ...
          <>
            ...
            >
              ...
              {!isFolder && cat === 'image' && ctxBtn(
                ...
                async () => {
                  setCtxMenu(null)
                  const tid = toast.loading('OCR 辨識中...')
                  try {
                    const r = await apiJson<{ ok: boolean; chars: number; preview: string }>(
                      '/api/ocr/index',
                      { method: 'POST', headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ path: ctxPath }) }
                    )
                    toast.dismiss(tid)
                    toast.success(`已建立索引：${r.chars} 字（${r.preview.slice(0,30)}...）`)
                  } catch (e) {
                    toast.dismiss(tid)
                    toast.error((e as Error).message)
                  }
              ...
              {isFolder && ctxBtn(
                ...
                async () => {
                  setCtxMenu(null)
                  const r = await apiJson<{ ok: boolean; queued: number }>(
                    '/api/ocr/index-folder',
                    { method: 'POST', headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ path: ctxPath }) }
                  ).catch(e => { toast.error((e as Error).message); return null })
                  if (r) toast.success(`已排程 ${r.queued} 張圖片進行 OCR 索引（背景執行）`)
                }
              )}

              {!isFolder && cat === 'video' && ctxBtn(
                '轉換為圖片',
                ...
                async () => {
                  ...
                  try {
                    ...
                    toast.dismiss(tid)
                    if (r.type === 'single') {
                      toast.success(`已轉換為圖片：${r.output}（${r.duration.toFixed(2)}秒）`)
                    } else {
                      toast.success(`已擷取 ${r.count} 張圖片（每秒一幀）`)
                    }
                    await loadItems()
                  } catch (e) {
                    toast.dismiss(tid)
