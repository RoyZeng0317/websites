...
export default function SearchBar({ onNavigate }: SearchBarProps) {
  ...
  const [ocrSearch, setOcrSearch] = useState(true)
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(async () => {
      try {
        ...
        const [fileData, metaData, ocrData] = await Promise.all([
          apiJson<{ items: SearchResult[] }>(`/api/files/search?q=${q}`),
          apiJson<{ items: SearchResult[] }>(`/api/media/search?q=${q}`).catch(() => ({ items: [] })),
          ocrSearch
            ? apiJson<{ path: string; name: string; snippet: string }[]>(`/api/ocr/search?q=${q}`)
                .then(r => r.map(x => ({ ...x, type: 'file' as const, source: 'ocr' as const })))
                .catch(() => [])
            : Promise.resolve([]),
        ])
        ...
        for (const item of [
          ...fileData.items.map(i => ({ ...i, source: 'file' as const })),
          ...metaData.items.map(i => ({ ...i, source: 'meta' as const })),
          ...ocrData,
        ]) {
    ...
    <div className="relative">
      <div className={`flex items-center bg-gray-800 rounded-lg px-3 py-1.5 transition-all duration-200 ${
        ...
        <input
          ...
          placeholder={voiceActive ? '' : 'Search files, tags, locations…'}
          name="nas-search"
          autoComplete="off"
          autoCorrect="off"
          autoCapitalize="off"
        ...
        <button
          ...
          title={ocrSearch ? 'OCR content search: ON' : 'OCR content search: OFF'}
          className={`ml-1 shrink-0 text-[10px] px-1.5 py-0.5 rounded font-medium transition-colors ${
            ocrSearch ? 'bg-violet-600/30 text-violet-400' : 'text-gray-600 hover:text-gray-400'
          }`}
        >OCR</button>
        ...
        <button
          ...
          title={isRecording ? 'Stop recording' : isProcessing ? 'Processing…' : 'Voice search'}
          ...
          {results.map(item => (
            <button key={item.path} onClick={() => handleSelect(item)}
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.snippet && (
                  <p className="text-[10px] text-violet-300 mt-0.5 line-clamp-2 leading-relaxed">
                    <span className="text-violet-500">OCR: </span>{item.snippet}
                  </p>
                )}
