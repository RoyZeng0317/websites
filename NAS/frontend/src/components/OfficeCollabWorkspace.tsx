  ...
  return (
    <div className="h-full flex flex-col bg-gray-950">
      ...
      <div className="shrink-0 flex items-center gap-2 px-3 py-1.5 bg-gray-900 border-b border-gray-800">
        ...
        <span className="ml-auto text-xs text-gray-600">{numRows} 列 × {numCols} 欄</span>
        {editMode && (
      ...
      {cellCtx && (
        <>
          ...
          >
            ...
            {!editMode && (
              <p className="px-3 py-2 text-xs text-gray-600">請先開啟編輯模式</p>
            )}
          </div>
        </>
      )}
...
function DocumentSurface({ doc, onDownload }: { doc: OfficeDocument; onDownload: () => void }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!res.ok) throw new Error(`無法載入檔案 (HTTP ${res.status})`)
      ...
      if (doc.category === 'doc') {
        ...
        if (!cancelled) setHtml(result.value || '<p>（文件為空）</p>')
    ...
    return (
      <div className="h-full min-h-[400px] flex flex-col items-center justify-center gap-5 bg-gray-950 rounded-xl border border-gray-800 p-8 text-center">
        ...
        <div className="space-y-1">
          <p className="text-white font-medium">PowerPoint 無法在瀏覽器中預覽</p>
          <p className="text-sm text-gray-500">請下載後以桌面版 PowerPoint 開啟</p>
        </div>
  ...
  if (loading) {
    return (
      <div className="h-full min-h-[400px] flex flex-col items-center justify-center gap-3 bg-gray-950 rounded-xl border border-gray-800">
        ...
        <p className="text-sm text-gray-500">載入文件內容中…</p>
      </div>
    )
  }
  ...
  if (err) {
    return (
      <div className="h-full min-h-[400px] flex flex-col items-center justify-center gap-3 bg-gray-950 rounded-xl border border-red-900/40 p-8 text-center">
        <p className="text-red-400 font-medium">無法載入文件</p>
    ...
    return (
      <div className="h-full min-h-[400px] bg-gray-100 rounded-xl overflow-hidden flex flex-col">
        <div className="shrink-0 flex items-center gap-2 px-4 py-2 bg-white border-b border-gray-200">
          ...
          {editMode && <span className="text-xs text-amber-600">注意：將儲存為 .html 格式</span>}
        </div>
  ...
  return (
    <div className="h-full min-h-[400px] bg-gray-950 rounded-xl border border-gray-800 overflow-hidden flex flex-col">
      ...
      <div className="shrink-0 flex items-center gap-2 px-3 py-2 bg-gray-900 border-b border-gray-800">
        ...
        >{editMode ? '📝 編輯中' : '閱讀模式'}</button>
        {editMode && <span className="text-xs text-gray-500">右鍵儲存格可插入／刪除列欄</span>}
      ...
      {showOpenMenu && (
        <>
          ...
          <div className="fixed z-50 bottom-16 left-1/2 -translate-x-1/2 bg-gray-800 border border-gray-700 rounded-xl shadow-2xl p-4 w-80 space-y-3">
            <p className="text-sm font-semibold text-white">在桌面版 Excel 開啟</p>
            <div className="bg-yellow-900/30 border border-yellow-800/40 rounded-lg px-3 py-2 text-xs text-yellow-300">
              Microsoft Office 可能封鎖樹莓派的 WebDAV 存取。建議改用「下載後開啟」。
            </div>
...
export default function OfficeCollabWorkspace({
  ...
  function sendCollab(payload: Record<string, unknown>): boolean {
    ...
    if (!ws || ws.readyState !== WebSocket.OPEN) { toast.error('協作連線尚未就緒'); return false }
  ...
  async function createEditShare(doc: OfficeDocument) {
    ...
    try {
      ...
      await copyText(`${window.location.origin}/share/${token}`, '協作連結')
    } catch (err: unknown) {
      toast.error((err as { message?: string }).message ?? '建立連結失敗')
      ...
      <header className="h-14 shrink-0 flex items-center justify-between px-4 border-b border-gray-800 bg-gray-900">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-sm font-semibold text-white truncate">Office 協作工作區</h2>
            <p className="text-xs text-gray-500 truncate">{currentPath ? `/${currentPath}` : '/'} · {connected ? '即時協作' : '連線中'}</p>
          </div>
        </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row">
        <aside className="lg:w-72 shrink-0 border-b lg:border-b-0 lg:border-r border-gray-800 bg-gray-900/70 flex flex-col max-h-48 lg:max-h-none">
          <div className="px-4 py-3 border-b border-gray-800 flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Office 檔案</span>
            <span className="text-xs text-gray-600">{availableDocs.length}</span>
          </div>
          <div className="overflow-y-auto p-2 space-y-1">
            {availableDocs.length === 0 ? (
              <div className="px-3 py-8 text-center text-sm text-gray-600">目前資料夾沒有 Office 檔案</div>
              ...
              return (
                <button key={doc.path} onClick={() => setActivePath(doc.path)}
                  ...
                  <span
                    ...
                    className="w-5 h-5 shrink-0 flex items-center justify-center rounded hover:bg-white/10" title="關閉分頁">
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {activeDoc && activeDetails ? (
              ...
              <section className="min-w-0 min-h-0 flex flex-col">
                <div className="shrink-0 px-5 py-4 border-b border-gray-800 flex flex-col md:flex-row md:items-center gap-3 md:justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-2 relative">
                      ...
                      {showOpenMenu && (
                        <>
                          ...
                          <div className="absolute right-0 top-10 z-50 w-72 bg-gray-800 border border-gray-700 rounded-xl shadow-2xl p-3 space-y-2">
                            <div className="bg-yellow-900/30 border border-yellow-800/40 rounded-lg px-3 py-2 text-xs text-yellow-300">
                              Microsoft Office 可能封鎖樹莓派的 WebDAV 存取，建議下載後再以桌面版開啟。
                            </div>
                            ...
                            <button onClick={() => { copyText(webDavUrl(activeDoc.path), 'WebDAV 網址'); setShowOpenMenu(false) }}
                              className="w-full py-2 rounded-lg bg-gray-700 hover:bg-gray-600 text-gray-200 text-xs transition-colors">
                              複製 WebDAV 網址（貼入 Office 開啟對話框）
                            </button>
                    ...
                    <button onClick={() => copyText(webDavUrl(activeDoc.path), 'WebDAV 連結')}
                ...
                <div className="shrink-0 px-5 py-3 border-t border-gray-800 bg-gray-900/80 flex flex-wrap items-center gap-3">
                  <a href={downloadUrl(activeDoc.path)} target="_blank" rel="noopener noreferrer"
                    className="text-xs text-gray-500 hover:text-gray-300 transition-colors">瀏覽器連結</a>
              ...
              <aside className="min-h-0 border-t xl:border-t-0 xl:border-l border-gray-800 bg-gray-900 flex flex-col">
                <div className="shrink-0 px-4 py-3 border-b border-gray-800">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-gray-400 uppercase tracking-wide">協作者</span>
                    <span className={`text-xs ${connected ? 'text-emerald-400' : 'text-yellow-400'}`}>
                      {connected ? `${collaborators.length} 人在線` : '連線中'}
                    </span>
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {collaborators.length === 0 ? (
                      <span className="text-xs text-gray-600">尚無成員</span>
                ...
                <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-5">
                  <section>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-sm font-semibold text-white">待辦</h4>
                      <span className="text-xs text-gray-600">{tasks.filter(t => !t.done).length}</span>
                    </div>
                    <form onSubmit={addTask} className="flex gap-2 mb-3">
                      <input value={newTask} onChange={e => setNewTask(e.target.value)} placeholder="新增任務"
                      ...
                      <button className="h-9 px-3 rounded-lg bg-orange-500 hover:bg-orange-600 text-white text-xs font-medium transition-colors">新增</button>
                    </form>
                    <div className="space-y-2">
                      {tasks.length === 0 ? (
                        <p className="text-xs text-gray-600 py-2">沒有待辦</p>
                      ) : tasks.map(task => (
                  ...
                  <section>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-sm font-semibold text-white">批註</h4>
                      <span className="text-xs text-gray-600">{notes.length}</span>
                    </div>
                    <form onSubmit={addNote} className="space-y-2 mb-3">
                      <textarea value={newNote} onChange={e => setNewNote(e.target.value)} placeholder="新增批註" rows={3}
                      ...
                      <button className="w-full h-9 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-200 text-xs transition-colors">送出</button>
                    </form>
                    <div className="space-y-2">
                      {notes.length === 0 ? (
                        <p className="text-xs text-gray-600 py-2">沒有批註</p>
                  ...
                  {activities.length > 0 && (
                    <section>
                      <h4 className="text-sm font-semibold text-white mb-2">動態</h4>
                      <div className="space-y-1">
            ...
            <div className="flex-1 flex items-center justify-center p-6">
              <div className="text-center space-y-3">
                ...
                <p className="text-sm text-gray-500">沒有開啟的 Office 檔案</p>
              </div>
            </div>
          )}
        </main>
